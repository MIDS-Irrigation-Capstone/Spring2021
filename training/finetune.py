...
def finetune_pretrained_model(model, num_unfrozen, metrics=METRICS):
  ...
  # Add output layer
  output = tf.keras.layers.Dense(1, activation='sigmoid', name='dense_output')(pretrained_model.layers[-1].output)
...
def run_model(name, output_dir, pretrained_model, BATCH_SIZE, epochs, training_dataset, valid_dataset, NUM_UNFROZEN, expanded_labels=False):
    print(50 * "*")
    ...
    training_data = get_dataset(training_dataset, batch_size=BATCH_SIZE, expanded=expanded_labels)
    val_data = get_dataset(valid_dataset, batch_size=BATCH_SIZE, expanded=expanded_labels)
